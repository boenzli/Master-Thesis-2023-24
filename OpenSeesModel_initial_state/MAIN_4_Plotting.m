...
% Specify the analysis you want to plot
EQdir = ["Y"]; % Capital X or Y  to specify the direction
EQfac = [2.0]; % Needs to correspond to one of the analysis that were performed
for a = 1:1:length(EQdir)
    for ii = 1:1:length(EQfac)
        ...
        try
            ...
            % workspace = strcat("ParishHouse_EQdirX_EQfac1.mat");
            load(workspace);
            load("modelParameters.mat")
            ...
            % title ("Modal shapes");
            % 
            % modalShapeName = strcat(Results,'/modal_shape.fig');% Specify the filename with the .fig extension
            % savefig(modalShapeName);% Save the figure as a MATLAB figure file
            ...
            % selfweightShapeName = strcat(Results,'/selfweight_shape.fig');% Specify the filename with the .fig extension
            % savefig(selfweightShapeName);% Save the figure as a MATLAB figure file
            ...
            % driftFName = strcat(Results,'/driftF_shape.fig');% Specify the filename with the .fig extension
            % savefig(driftFName);% Save the figure as a MATLAB figure file
            ...
            % driftSName = strcat(Results,'/driftS_shape.fig');% Specify the filename with the .fig extension
            % savefig(driftSName);% Save the figure as a MATLAB figure file
